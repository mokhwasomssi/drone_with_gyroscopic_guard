...
{
	HAL_TIM_PWM_Start_DMA(dshot_handle->dshot_timer, dshot_handle->channel, dshot_handle->dshot_dmabuffer, 18);
...
void dshot_dma_start(TIM_HandleTypeDef *htim, uint32_t channel, uint32_t *pData, uint16_t Length)
{
    ...
    switch (channel) {
    case TIM_CHANNEL_1:
        HAL_DMA_Start(htim->hdma[TIM_DMA_ID_CC1], (uint32_t)pData, (uint32_t)&htim->Instance->CCR1, Length);
        __HAL_TIM_ENABLE_DMA(htim, TIM_DMA_CC1);
    break;

    case TIM_CHANNEL_2:
        HAL_DMA_Start(htim->hdma[TIM_DMA_ID_CC2], (uint32_t)pData, (uint32_t)&htim->Instance->CCR2, Length);
        __HAL_TIM_ENABLE_DMA(htim, TIM_DMA_CC2);
        break;

    case TIM_CHANNEL_3:
        HAL_DMA_Start(htim->hdma[TIM_DMA_ID_CC3], (uint32_t)pData, (uint32_t)&htim->Instance->CCR3,Length);
        __HAL_TIM_ENABLE_DMA(htim, TIM_DMA_CC3);
        break;

    case TIM_CHANNEL_4:
        HAL_DMA_Start(htim->hdma[TIM_DMA_ID_CC4], (uint32_t)pData, (uint32_t)&htim->Instance->CCR4, Length);
...
void dshot_write_all(dshot_handle_t *dshot_handle_array, uint16_t *value)
{
	...
	for(int i = 0; i < 4; i++)
	{
		dshot_dma_start_hal(&dshot_handle[i]);
	}
